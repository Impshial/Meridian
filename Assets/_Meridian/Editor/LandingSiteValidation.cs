    ...
    public static class LandingSiteValidation
    {
        ...
        public static void CameraBounds()
        {
            ...
            try
            {
                ...
                {
                    camera.aspect=aspect;view.Initialize(camera,new Rect(-1000,-1000,2000,2000),Vector3.zero,(x,z)=>0,null);
                    ...
                    foreach(float edgeX in new[]{-10000f,10000f})foreach(float edgeZ in new[]{-10000f,10000f})
                    {
                        ...
                        for(int heading=0;heading<360;heading+=45)for(int tilt=42;tilt<=78;tilt+=6)
                        {
                            ...
                            for(int x=0;x<=1;x++)for(int y=0;y<=1;y++)
                            {
                                ...
                                Require(Mathf.Abs(hit.x)<=1000 && Mathf.Abs(hit.z)<=1000,"Survey corner crossed the generated terrain boundary.");
                            }
                        }
                    }
                    // Reset should frame a landing area near the survey edge without moving its focus.
                    var nearEdge=new Vector3(650,0,-650);
                    view.Initialize(camera,new Rect(-1000,-1000,2000,2000),nearEdge,(x,z)=>0,null);
                    Require(Vector2.Distance(new Vector2(view.Pivot.x,view.Pivot.z),new Vector2(650,-650))<.001f,"Reset lost a landing area near the edge.");
                }
                Directory.CreateDirectory("Logs");
