...
CONTINUE becomes available after selecting land. It immediately shows `Loading Landing Site...`, then waits for numerical generation, terrain, colliders, materials, camera, and controls. Stage and percentage updates show actual preparation progress. BACK TO PLANET can cancel preparation and return to the same globe. A failed or timed-out survey also offers recovery; an independent 180-second watchdog covers scene activation, preparation and input release, even if the transition coroutine stops. Loading text and its unscaled animation have their own presentation above the fading black overlay. Held input is released before destination interaction is enabled.
...
Readiness rows and numerical tiles run on at most four workers, leaving capacity for the UI. Workers read frozen geography and own their output arrays. Results are published in stable tile-address order; Unity resource creation stays on the main thread. Cancellation is checked inside worker loops, and late results are rejected. Surface version 6 and its sampled geography are unchanged by parallel execution. A representative production Windows-player load decreased from 116.99 s to 42.64 s; see the dated validation record for scope.
