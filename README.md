...
Milestone 3 completes pregame landing selection. **NEW COLONY** immediately shows `Loading Exo-planet...` and creates a fresh globe. Clicks smoothly center a region; land clicks plant a readable flag and enable **CONTINUE**. It shows `Loading Landing Site...` and opens the matching terrain survey. Lock a valid dropship position and choose **LAND HERE** to confirm. **BACK TO PLANET** restores the same globe, view and flag. LOAD COLONY, SETTINGS and QUIT remain inert placeholders.
...
The initial surface is **2 km × 2 km**, made from four connected 1 km Unity Terrains with 513 × 513 heights each. Surface generator version 4 substantially reduces elevation and hills, broadens the level building area, and retains dense timber groves. Ground materials have 1024 × 1024 albedo, normal and surface maps with fine grass and mineral detail. Trees carry stable IDs and wood amounts; grove totals describe available generated members. Readiness requires at least 90,000 m² of connected gentle ground, a 200 m interior and a valid full landing/deployment footprint. Narrow or unsuitable regions recover to the planet without reseeding.

Surface controls: WASD/arrows or middle-drag pan horizontally, wheel tilts the overhead view, **Shift + wheel zooms**, Q/E turns the camera in 45° steps, +/− also zooms, and Home resets. Left-click locks a valid site, R/Shift+R changes the ship heading, and right-click/Esc clears. The camera uses one distance limit safe for every allowed heading and pitch—approximately 500 m at 16:9—so tilting does not change zoom. Confirmation retains the marker and survey controls; dropship arrival and colony simulation come later. Settings: `Assets/_Meridian/Settings/SurfaceGeneration.asset`. Authoring: `Meridian > Author Landing Site Selection`.
